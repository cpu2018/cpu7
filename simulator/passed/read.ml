...
let rec read_screen_settings _ =
  ...
  (* スクリーン中心の座標 *)
  screen.(0) <- read_float ();
  print_int (int_of_float screen.(0)); print_char 32;
  screen.(1) <- read_float ();
  print_int (int_of_float screen.(1)); print_char 32;
  screen.(2) <- read_float ();
  print_int (int_of_float screen.(2)); print_char 32;
  (* 回転角 *)
  let v1 = rad (read_float ()) in
  print_int (int_of_float (unrad v1)); print_char 32;
  let cos_v1 = cos v1 in
  let sin_v1 = sin v1 in
  let v2 = rad (read_float ()) in
  print_int (int_of_float (unrad v2)); print_char 10;
...
let rec read_light _ =
  ...
  light.(2) <- cl1 *. cl2;
  beam.(0) <- read_float ();
  print_int (int_of_float beam.(0)); print_char 10
